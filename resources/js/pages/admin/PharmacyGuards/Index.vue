    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="relative flex h-full flex-1 flex-col gap-4 overflow-x-auto p-4 md:p-6">
            ...
            <div class="relative rounded-2xl border border-sidebar-border/70 bg-background/95 p-6 shadow-sm">
                ...
                <Form v-bind="storePharmacyGuard.form()" :reset-on-success="['date', 'pharmacy_id']"
                    @success="filterGuards" v-slot="{ errors, processing }" class="mt-4 grid gap-5">
                    ...
                    <div class="grid gap-2">
                        ...
                        <select id="pharmacy_id" name="pharmacy_id" required
                            ...
                            <option v-for="pharmacy in props.pharmacies" :key="pharmacy.id"
            ...
            <div class="relative rounded-2xl border border-sidebar-border/70 bg-background/95 p-5 shadow-sm">
                ...
                <div class="relative mt-2">
                    <svg class="pointer-events-none absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-muted-foreground"
                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="m21 21-4.35-4.35m1.85-5.15a7 7 0 1 1-14 0 7 7 0 0 1 14 0Z" />
                    </svg>
                    <input v-model="searchquery" @keyup="filterGuards()" aria-labelledby="search" type="search"
                ...
                <div class="mt-4 overflow-hidden rounded-xl border border-sidebar-border/70">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-sidebar-border/70 text-sm">
                            ...
                            <tbody class="divide-y divide-sidebar-border/70">
                                <tr v-for="guard in visibleGuards" :key="guard.id"
                    ...
                    <div v-if="totalPages > 1"
                        class="flex items-center justify-between border-t border-sidebar-border/70 px-4 py-4">
                        ...
                        <div class="flex gap-2">
                            <button type="button" @click="changePage(currentPage - 1)" :disabled="currentPage <= 1"
                                class="rounded border border-sidebar-border px-3 py-1 hover:bg-muted disabled:opacity-30">
                                &lt;
                            </button>

                            <button type="button" class="rounded bg-secondary px-3 py-1 text-black">
                                {{ currentPage }}
                            </button>
